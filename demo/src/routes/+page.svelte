<script lang="ts">
	...
	const revealText = "Each word starts obscured by a block. As you scroll, the block covering each word shrinks away horizontally, revealing the text. This entire sequence completes as the paragraph reaches the middle of the viewport.";
	const wordsAndSpaces = revealText.split(/(\s+)/);
	const imgSrc = 'https://plus.unsplash.com/premium_photo-1670590820797-51389452a3c3?q=80&w=2670&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'
...
<main class="page-content">
	<header class="intro-header">
		...
		<nav>
			<a href="[YOUR_GITHUB_REPO_URL]" target="_blank" rel="noopener noreferrer">GitHub</a>
			<a href="[YOUR_NPM_PACKAGE_URL]" target="_blank" rel="noopener noreferrer">npm</a>
		</nav>
	</header>

	<div class="instructions">
		<p>Scroll down to see different effects powered by the <code>--scroll-progress</code> CSS variable (0 to 1).</p>
	</div>

	 <section class="demo-section">
        <h2>Example 1: Fade & Slide Up (Standard Range 0-1)</h2>
        <ScrollTracker debug={true}>
            <article class="content-box animate-fade-slide">
                <div class="effect-label">Fade & Slide Up</div>
                <h3>Smooth Entrance</h3>
                <p>The most basic effect, fading and sliding into view smoothly across the viewport scroll.</p>
    ...
    <section class="demo-section">
        ...
        <ScrollTracker startThreshold={0} endThreshold={0.4}>
            <article class="content-box animate-scale">
                <div class="effect-label">0-40% Journey4</div>
                <h3>Quick Growth</h3>
                <p>This element scales in and moves up, completing its animation by the time it reaches the middle of the screen.</p>
            </article>
        </ScrollTracker>
    </section>
    ...
    <section class="demo-section">
        ...
        <ScrollTracker startThreshold={0.2} endThreshold={0.8}>
            <article class="content-box animate-color">
                ...
                <p>The background color shifts only during the middle part of its journey across the viewport.</p>
            </article>
        </ScrollTracker>
    </section>
    ...
    <section class="demo-section">
        ...
        <ScrollTracker startThreshold={0.1} endThreshold={0.9}>
             <article class="content-box animate-svg">
                 ...
                 <h3>Drawing with Scroll</h3>
                 <p>Animating SVG stroke properties.</p>
                 <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
                    <path class="svg-path" d="M 10 10 H 90 V 90 H 10 L 10 10" fill="none" stroke="currentColor" stroke-width="4"/>
                 </svg>
             </article>
        </ScrollTracker>
    </section>
...
<style lang="scss">
	...
	.content-box {
     ...
     transition: opacity 0.2s ease-out, transform 0.2s ease-out, background-color 0.2s ease-out, width 0.2s ease-out, clip-path 0.2s ease-out, stroke-dashoffset 0.1s linear;
		:global(.dark) & {
			--color-box-bg: #111;
		}
		h2 {
	...
	.effect-label {
		...
		font-size: 0.8rem;
		font-weight: 600;
		border-radius: 0 var(--border-radius) 0 var(--border-radius); /* Top-right, bottom-left */
        font-family: var(--font-code);
	}
	...
	* :global(.animate-scale) {
		...
		transform:
            /* Scale from 0.9 to 1.0 as progress goes 0 to 1 */
            scale(calc(0.9 + 0.1 * var(--scroll-progress)))
	...
	* :global(.animate-color) {
		...
		:global(h2), :global(p) { color: var(--color-text); } /* Ensure text stays readable */
	}
	...
	* :global(.animate-reveal) {
	 ...
	 * :global(.animate-width) {
        ...
        :global(p) { white-space: nowrap; } /* Prevent text wrapping during animation */
	 }
